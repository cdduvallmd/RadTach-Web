...
// ============================================================================
//
// RadTach can operate semi-automatically if a local middleware agent provides
// real-time signals from the hospital HL7 feed and/or dictation system.
//
// The middleware would be a small Python service running on the reading room
// workstation, communicating with RadTach via WebSocket on localhost. It
// extracts ONLY the fields listed below — no MRN, no patient name, no PHI,
// no protected health information of any kind.
//
// ── INBOUND SIGNALS (middleware → RadTach) ──────────────────────────────────
//
//   Signal              Source        Data Fields         RadTach Action
//   ─────────────────── ───────────── ─────────────────── ──────────────────────
//   NEW_STUDY_OPENED    HL7 ORM/ORU  AE Title,           Auto-select modality
//                                     Accession#,         via AE Title → modality
//                                     CPT Code            lookup table, store
//                                                         Accession# in ref,
//                                                         look up RVU from CPT
//                                                         → setSelectedModality()
//
//   SAME_STUDY_REOPENED HL7 ORM/ORU  Accession#          If Accession# matches
//                       (derived)     (matches stored)    stored value:
//                                                         → toggleDoubleTap()
//
//   DICTATION_STARTED   Dictation    (none)              Start study timer
//                       system API                        → toggleTimer()
//
//   REPORT_DRAFTED      Dictation    (none)              Complete current study
//                       system API                        → completeStudy()
//
//   REPORT_SIGNED       Dictation    (none)              Complete current study
//                       system API                        (if not already)
//                                                         → completeStudy()
//
// ── DATA FIELDS EXTRACTED (exhaustive list) ─────────────────────────────────
//
//   Field           HL7 Segment    Purpose                     PHI?
//   ─────────────── ────────────── ─────────────────────────── ────
//   AE Title        OBR-24 or     Maps to modality (CT, MR,   No
//                   custom field   XR, etc.) via site-specific
//                                  lookup table
//   Accession#      OBR-18        Detects reopened studies     No*
//                                  (Double Tap). Stored only
//                                  in memory (useRef), never
//                                  persisted or transmitted.
//   CPT Code        OBR-4 or      Accurate RVU lookup via     No
//                   OBR-44         CMS Medicare Physician Fee
//                                  Schedule. Enables precise
//                                  Professional Component RVU
//                                  instead of modality-based
//                                  estimates. Critical for
//                                  administrative reports and
//                                  contract compliance.
//   Dictation state RIS/PACS API  Start, draft, sign events   No
//
//   * Accession# is a study identifier, not a patient identifier. It is
//     never stored in Firestore, localStorage, or any persistent storage.
//     It exists only as a transient comparison value in browser memory.
//
// ── WHY CPT CODE MATTERS ──────────────────────────────────────────────────
//
//   Without HL7 integration, RadTach estimates RVU from modality and
//   complication selections. These estimates run 10-20% below actual RVU
//   values (as reported by Epic, Medicalis, or other billing systems).
//
//   With CPT code extraction, RadTach can look up the exact Professional
//   Component RVU from the CMS Medicare Physician Fee Schedule (MPFS),
//   published annually as a free public dataset. This closes the accuracy
//   gap entirely and makes RadTach RVU data reliable for:
//     - Radiologist self-assessment (accurate daily/weekly totals)
//     - Administrative reports (RVU vs. contracted targets)
//     - Group productivity dashboards
//     - Staffing and rotation analysis
//
//   The MPFS RVU table (~500 radiology CPT codes, 70000-79999 range)
//   would be stored in Firestore (config/rvuTable/{year}), updated
//   annually when CMS publishes the new fee schedule (typically November
//   for the following calendar year).
//
//   CPT codes are procedure identifiers, not patient identifiers. No PHI.
//
// ── AE TITLE → MODALITY MAPPING ────────────────────────────────────────────
//
//   AE Titles vary by site. Examples:
//     "CT_SCANNER_1", "GE_CT_BAY3", "CTSCANNER" → CT
//     "MR_3T_ROOM2", "SIEMENS_MRI" → MR
//     "DR_ROOM1", "XRAY_ER" → XR
//
//   Mapping table would be stored in Firestore (config/aeTitleMappings),
//   editable by site admins. The middleware performs the lookup locally
//   using a cached copy of the mapping.
//
// ── ARCHITECTURE ────────────────────────────────────────────────────────────
//
//   Hospital Network          Workstation              Browser
//   ┌─────────────┐     ┌──────────────────┐     ┌──────────────┐
//   │  HL7 Feed   │────▶│  Python Agent    │────▶│   RadTach    │
//   │  (MLLP/TCP) │     │  (localhost)     │ WS  │   (web app)  │
//   └─────────────┘     │                  │     │              │
//   ┌─────────────┐     │  Extracts:       │     │  Receives:   │
//   │  Dictation  │────▶│  - AE Title      │     │  - modality  │
//   │  System API │     │  - Accession#    │     │  - CPT code  │
//   └─────────────┘     │  - CPT Code      │     │  - signals   │
//                       │  - Dict. state   │     └──────────────┘
//                       └──────────────────┘
//                       NO PHI leaves this box.
//                       NO PHI enters the browser.
//                       NO PHI is stored anywhere.
//
// ── HOOK POINTS IN THIS FILE ────────────────────────────────────────────────
//
//   setSelectedModality()  — modality selection (button click handler)
...
// ── NOT IN SCOPE ────────────────────────────────────────────────────────────
//
//   - Patient name, MRN, DOB, or any demographic data
//   - Study description or clinical indication
//   - Images or pixel data
//   - Report text or findings
//   - Any data that would require IRB approval or BAA
//
// ── REFERENCE IMPLEMENTATION — HL7 Middleware Agent (Python) ────────────────
//
//   The following is a complete, runnable Python agent that demonstrates
//   the HL7 listener described above. It uses standard libraries plus two
//   small packages (python-hl7 for parsing, websockets for browser comms).
//
//   Install: pip install python-hl7 websockets
//
//   This code is provided for IT security review. It can be audited
//   line-by-line to confirm that no PHI is extracted, stored, or
//   transmitted. The agent reads exactly three fields from each HL7
//   message (AE Title, Accession#, and CPT Code) and discards
//   everything else.
//
// --------------------------------------------------------------------------
//
//   """
//   radtach_hl7_agent.py — HL7 Middleware Agent for RadTach
//
//   Listens for HL7 v2.x messages over MLLP (TCP), extracts only:
//     - AE Title (OBR-24): mapped to radiology modality
//     - Accession# (OBR-18): used to detect reopened studies
//     - CPT Code (OBR-4): used for accurate RVU lookup via CMS MPFS
//
//   Sends signals to RadTach (browser) via WebSocket on localhost.
//   NO PHI is extracted, stored, logged, or transmitted.
//
//   Usage:
//     python radtach_hl7_agent.py --hl7-port 2575 --ws-port 8765
//   """
//
//   import asyncio
//   import argparse
...
//   log = logging.getLogger("radtach-agent")
//
//   # ── AE Title → RadTach Modality Mapping ──────────────────────────────
//   # Site-specific. Each key is a substring to match against OBR-24.
//   # In production this would be loaded from Firestore config/aeTitleMappings.
//   AE_TITLE_MAP = {
//       "CT":  "CT",
//       "MR":  "MR",  "MRI": "MR",
//       "XR":  "XR",  "DR":  "XR",  "CR": "XR", "XRAY": "XR",
//       "US":  "US",  "ULTRA": "US",
//       "FL":  "FL",  "FLUORO": "FL",
//       "NM":  "NM",  "NUC": "NM",
//       "MA":  "MA",  "MAMMO": "MA",
//       "PT":  "PET-CT",  "PET": "PET-CT",
//   }
//
//
//   def match_modality(ae_title: str) -> str | None:
//       """Match an AE Title string to a RadTach modality code."""
//       upper = ae_title.upper()
//       for keyword, modality in AE_TITLE_MAP.items():
//           if keyword in upper:
//               return modality
//       return None
//
//
//   def extract_cpt(obr4_value: str) -> str | None:
//       """
//       Extract CPT code from OBR-4 (Universal Service Identifier).
//       OBR-4 is a Coded Element (CE) with components separated by ^:
//         component 1 = identifier (may be CPT or internal order code)
//         component 4 = alternate identifier (may contain CPT)
//       Sites vary: some put CPT in component 1, others in component 4.
//       We check both, preferring a value that looks like a radiology CPT
//       (5-digit number in the 70000-79999 range).
//       """
//       if not obr4_value:
//           return None
//       components = obr4_value.split("^")
//       candidates = []
//       if len(components) > 0 and components[0].strip():
//           candidates.append(components[0].strip())
//       if len(components) > 3 and components[3].strip():
//           candidates.append(components[3].strip())
//       for c in candidates:
//           # Check if it looks like a radiology CPT code
//           if c.isdigit() and len(c) == 5 and c.startswith("7"):
//               return c
//       # Fallback: return the first component even if it doesn't match
//       # the expected pattern (site may use non-standard coding)
//       return candidates[0] if candidates else None
//
//
//   # ── MLLP Protocol ────────────────────────────────────────────────────
//   # HL7 v2.x messages are wrapped in MLLP framing:
//   #   \x0b (VT)  = start of message
//   #   \x1c\x0d   = end of message (FS + CR)
//   MLLP_START = b"\x0b"
//   MLLP_END = b"\x1c\x0d"
//
//
...
//   last_accession: str | None = None   # Most recent Accession# (memory only)
//
//
...
//         - Accession# (OBR-18)
//         - CPT Code (OBR-4)
//       All other fields — including PID (patient data) — are ignored.
//       """
//       try:
//           msg = hl7.parse(raw)
//       except Exception:
//           log.warning("Failed to parse HL7 message — skipping")
//           return None
//
//       # Find the OBR segment (Observation Request)
//       obr = None
//       for segment in msg:
...
//
//       if obr is None:
//           return None  # No OBR segment — not a study message
//
//       # Extract the three fields we need. Nothing else.
//       accession = str(obr[18]).strip() if len(obr) > 18 else ""
//       ae_title = str(obr[24]).strip() if len(obr) > 24 else ""
//       cpt_raw = str(obr[4]).strip() if len(obr) > 4 else ""
//       cpt_code = extract_cpt(cpt_raw)
//
//       if not accession and not ae_title and not cpt_code:
//           return None  # Nothing useful in this message
//
//       return {"accession": accession, "ae_title": ae_title, "cpt": cpt_code}
//
//
//   async def broadcast(message: dict):
//       """Send a JSON message to all connected RadTach browser clients."""
...
//
//
//   async def process_hl7(raw: str):
//       """Process one HL7 message and broadcast signals to RadTach."""
//       global last_accession
//
//       fields = parse_hl7_message(raw)
//       if fields is None:
//           return
//
//       accession = fields["accession"]
//       ae_title = fields["ae_title"]
//       cpt_code = fields["cpt"]
//       modality = match_modality(ae_title) if ae_title else None
//
//       # Determine signal type
//       if accession and accession == last_accession:
//           # Same accession as last study — this is a reopened study
//           log.info(f"SAME_STUDY_REOPENED (accession repeated)")
//           await broadcast({
//               "signal": "SAME_STUDY_REOPENED",
//           })
//       else:
//           # New study
//           last_accession = accession if accession else last_accession
//           log.info(f"NEW_STUDY_OPENED modality={modality} cpt={cpt_code}")
//           await broadcast({
//               "signal": "NEW_STUDY_OPENED",
//               "modality": modality,  # None if AE Title didn't match
//               "cpt": cpt_code,       # None if not found in OBR-4
//           })
//
//
//   # ── MLLP TCP Server ──────────────────────────────────────────────────
...
//       """Handle one MLLP TCP connection. Reads HL7 messages until closed."""
//       peer = writer.get_extra_info("peername")
//       log.info(f"MLLP connection from {peer}")
//       buffer = b""
//
//       try:
//           while True:
...
//               buffer += data
//
//               # Extract complete MLLP-framed messages from buffer
//               while MLLP_START in buffer and MLLP_END in buffer:
//                   start = buffer.index(MLLP_START) + 1
//                   end = buffer.index(MLLP_END)
//                   raw_hl7 = buffer[start:end].decode("utf-8", errors="replace")
//                   buffer = buffer[end + len(MLLP_END):]
//
//                   await process_hl7(raw_hl7)
//       except asyncio.CancelledError:
//           pass
//       finally:
//           writer.close()
//           log.info(f"MLLP connection closed from {peer}")
//
//
//   # ── WebSocket Server ─────────────────────────────────────────────────
//
//   async def handle_websocket(ws):
//       """Register a RadTach browser client."""
//       connected_clients.add(ws)
//       log.info(f"RadTach client connected ({len(connected_clients)} total)")
//       try:
//           async for _ in ws:
//               pass  # RadTach doesn't send us anything; we only push signals
//       finally:
//           connected_clients.discard(ws)
//           log.info(f"RadTach client disconnected ({len(connected_clients)} total)")
//
//
//   # ── Main ─────────────────────────────────────────────────────────────
//
//   async def main(hl7_port: int, ws_port: int):
//       mllp_server = await asyncio.start_server(handle_mllp_connection, "127.0.0.1", hl7_port)
//       log.info(f"MLLP listener on 127.0.0.1:{hl7_port}")
//
//       async with serve(handle_websocket, "127.0.0.1", ws_port):
//           log.info(f"WebSocket server on ws://127.0.0.1:{ws_port}")
//           await asyncio.Future()  # Run forever
//
//
//   if __name__ == "__main__":
//       parser = argparse.ArgumentParser(description="RadTach HL7 Agent")
//       parser.add_argument("--hl7-port", type=int, default=2575)
//       parser.add_argument("--ws-port", type=int, default=8765)
//       args = parser.parse_args()
//       asyncio.run(main(args.hl7_port, args.ws_port))
//
// --------------------------------------------------------------------------
//
//   What this agent does NOT do:
//     - Does not read PID segment (patient name, MRN, DOB)
//     - Does not read OBX segment (results, impressions, report text)
//     - Does not log message contents (only signal type, modality, and CPT)
//     - Does not persist Accession# (held in memory, overwritten each study)
//     - Does not communicate with any server other than localhost
//     - Does not require network access beyond the hospital HL7 feed
//
//   What an IT auditor can verify in under 5 minutes:
//     1. parse_hl7_message() reads obr[4], obr[18], obr[24] — nothing else
//     2. broadcast() sends only "signal", "modality", "cpt" — no HL7 content
//     3. last_accession is a Python variable, never written to disk
//     4. Both servers bind to 127.0.0.1 — no external network exposure
//     5. CPT codes are procedure identifiers, not patient identifiers (no PHI)
//
// ============================================================================
